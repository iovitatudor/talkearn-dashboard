<template>
  <div class="container">
    <div class="call ringing" v-if="isCalling">
      ...
      <div class="details">Calling to...</div>
      <ul class="actions">
        <li class="action"><a id="accept" href="#" @click="accept">Accept</a></li>
        <li class="action"><a id="refuse" href="#" @click="refuse">Refuse</a></li>
      </ul>
    </div>
...
export default {
  ...
  created() {
    this.sockets.subscribe(`outComingCall-${this.signId}`, (data) => {
      this.setRinging();
      console.log('outComingCall...');
    });
  },
  methods: {
    setRinging() {
    ...
    accept() {
      this.$el.querySelector(".ringing").classList.remove('-ringing');

      setTimeout(() => {
        this.$el.querySelector(".ringing").classList.add('-flip');
    ...
    hold() {
      this.$el.querySelector(".ringing").classList.toggle(hold);
      // this.onPause($('.speaking').hasClass('hold'));
    },
    ...
    refuse() {
      ...
      setTimeout(() => {
        this.$el.querySelector(".ringing").classList.add('-fadeout');
      }, 0);
    ...
    drop() {
      ...
      setTimeout(() => {
        this.$el.querySelector('ringing').classList.remove('-flip');
        this.$el.querySelector('speaking').classList.add('flipback');
        this.$el.querySelector('speaking').classList.remove('hold');
